...
if size(self.conv_coeff,1)==1; self.conv_coeff = self.conv_coeff*ones(length(self.id),length(faceNum)); end
if size(self.conv_ambient,1)==1; self.conv_ambient = self.conv_ambient*ones(length(self.id),length(faceNum)); end
if size(self.rad_coeff,1)==1; self.rad_coeff = self.rad_coeff*ones(length(self.id),length(faceNum)); end
if size(self.rad_ambient,1)==1; self.rad_ambient = self.rad_ambient*ones(length(self.id),length(faceNum)); end
...
for ii = 1:length(self.id)
    for jj = 1:length(faceNum)
        ...
        if ~isempty(self.conv_table_id) || any(self.conv_table_id~=0)
            [iErr] = calllib('St7API','St7SetBrickConvectionTables',uID,self.id(ii),...
                faceNum(jj),LCase,self.conv_table_id);
            HandleError(iErr)
        end
        if ~isempty(self.rad_table_id) || any(self.rad_table_id~=0)
            [iErr] = calllib('St7API','St7SetBrickRadiationTables',uID,self.id(ii),...
                faceNum(jj),LCase,self.rad_table_id);
            HandleError(iErr)
        end
    end
end
